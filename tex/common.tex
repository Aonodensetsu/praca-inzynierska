...
\section{Przegląd rynku}\label{sec:q}
...
pozwala~na wykorzystanie standardowej architektury procesora i~modułów komunikacyjnych (\emph{WiFi}, \emph{Bluetooth}).
...
pozwalających na~podłączenie dowolnych układów wykorzystujących zasilanie \qty{3.3}{\V} lub \qty{5}{\V}\@.
Stanowią dzięki~temu przystępną opcję zarówno do~nauki i~użytkowania systemów komputerowych jak~i~eksperymentów
...
Niektóre moduły nie~zostały uwzględnione w~procesie selekcji:
\begin{itemize}
...
\begin{itemize}
  \item Baud rate    -- Prędkość transmisji (ilość czasu przypisana jednemu bitowi);
...
W~pracy wykorzystany właśnie w~tym celu, co~ułatwia osobie prowadzącej zajęcia wstępną konfigurację, a~także czyszczenie
...
przykład trybu ciemnego, a~w~pozostałych przypadkach w~celu lepszego dopasowania kolorystycznego rysunki
będą~wykorzystywać tryb jasny.

TODO: rysunek, tryb ciemny
...
jako~załącznik w~tym~pliku PDF, jako~załącznik w~Archiwum Prac Dyplomowych, jak~i na~mojej~stronie
...
\subsection{Wprowadzenie}\label{subsec:entry}

TODO: Wspomnieć `laboratorium 0' i rozbudować.

Do pracy dołączony jest również instruktażowy plik łączenia modułów
\ax{polaczenia.pdf}{out/polaczenia.pdf}[mimetype=application/pdf], pomagający osobie prowadzącej zajęcia połączenie
elementów sprzętowych laboratoriów.
