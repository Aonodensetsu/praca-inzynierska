...
Urządzenia \emph{Raspberry Pi} wyposażone~są w~zestaw czterdziestu pinów\thinspace\cite{pinout} \emph{GPIO}
pozwalających na~budowę dowolnych połączeń wykorzystujących zasilanie \qty{3.3}{\V} lub \qty{5}{\V}\@.
...
\begin{itemize}
  \item Flick HAT -- firma producenta została rozwiązana\thinspace\cite{pisupply}, oficjalna dokumentacja ich~produktów
        nie~jest dostępna, a~konfiguracja nakładki nie~jest oczywista;
  \item Automation HAT Mini -- jest zaprojektowany do~pracy z~urządzeniami o~napięciu \qty{12}{\V} lub \qty{24}{\V},
        co~przy nieprawidłowym użytkowaniu może uszkodzić płytkę;
  \item Unicorn HAT -- jest wyposażony w~macierz diod $8\,\times\,8$, podobnie do~\emph{Sense HAT}, który~ma~szersze
        możliwości.
\end{itemize}
...
z~innymi urządzeniami\thinspace\cite{pinout}.
...
i~pełni funkcję adaptera tego~interfejsu na~własny system złączy\thinspace\cite{grovedesc}.
...
nośnym prostokątnym o~stałej częstotliwości i~amplitudzie jako~szerokości poszczególnych impulsów elektrycznych,
...
\emph{Inter-Integrated Circuit Sound}\thinspace\cite{i2s} nie~jest powiązane z~I²C, jednak ta~sama firma utworzyła
ich~specyfikacje.
...
W~momencie pisania pracy najnowsza wersja z~\num{1}~października~2025 opiera~się na~\emph{Debianie 13 Trixie}, i~ta
właśnie wersja jest~wspierana przez skrypt instalacyjny i~środowisko laboratoryjne.
...
w~systemach \emph{Unix}, \emph{Linux} oraz~\emph{MacOS}.
...
Część osób jest~również przeciwna użyciu przez~język białych znaków do~definicji bloków.

\subsection{rpi-lgpio}\label{subsec:lgpio}

Wiązania \emph{rpi-lgpio}\thinspace\cite{lgpio} języka~C do~użytkowania interfejsu \emph{GPIO} zastępujące bibliotekę
\emph{rpi-gpio}.
Wykorzystuje te same nazwy importowane i~sygnatury w~celu zapewnienia kompatybilności wstecznej.
Istnieje z~powodu usunięcia interfejsu \emph{sysfs} do \emph{GPIO} z~nowszych wersji jądra systemowego.
Zamiast przestarzałego interfejsu sysfs wykorzystuje nowszy plik urządzenia \emph{gpiochip}.
...
znaczących wad wcześniej istniejących rozwiązań\thinspace\cite{marimofaq}.
...
Wady, na~których rozwiązaniu skupia~się \emph{marimo}:
\begin{itemize}
  \item \textbf{Powtarzalność}: kod widoczny w~notatniku \emph{Jupyter} niekoniecznie odpowiada obecnemu stanowi
        projektu, co~sprawia,~że \qty{36}{\percent} nie~jest powtarzalnych\thinspace\cite{jupyfail}.
        \emph{Marimo} aktualizuje stan pamięci przy~wszelkich zmianach w~kodzie;
  \item \textbf{Łatwość utrzymania}: notatniki \emph{marimo} są~przechowywane jako~czyste programy \emph{Python},
        co~pozwala m.in.~na ich~bezproblemowe wersjonowanie;
  \item \textbf{Interaktywność}: \emph{marimo} zawiera funkcjonalność tworzenia elementów interfejsu użytkownika,
        automatycznie synchronizowanych w~kodzie przy~zmianie wartości;
  \item \textbf{Wykorzystanie wtórne}: notatniki \emph{marimo} mogą~być wykorzystane z~poziomu linii komend
        jak~zwyczajne skrypty w~języku \emph{Python}, można również zaimportować zdefiniowane w~nich symbole
        do~innych plików;
  \item \textbf{Udostępnianie}: każdy notatnik \emph{marimo} może~również być~uruchomiony jako~interaktywna aplikacja
...
jako~załącznik w~tym~pliku PDF, jako~załącznik w~Archiwum Prac Dyplomowych, a~także na~mojej~stronie
Internetowej\thinspace\cite{script}.
...
Dzięki użyciu stałych wersji programów ilość zmian niezbędnych do~przystosowania skryptu do~wychodzących w~przyszłości
wersje systemu powinna~być minimalna.
...
przy~wykorzystaniu możliwości środowiska \emph{marimo}.
...
Dotyczy~to załączonego przeze~mnie skryptu instalacyjnego.
...
\begin{itemize}
  ...
  \item Wykorzystanie stałej wersji języka \emph{Python};
  \item Wykorzystanie środowiska wirtualnego;
  \item Wykorzystanie stałej wersji bibliotek języka;
  \item Załączona~instrukcja połączeń dla~prowadzącego;
  ...
  \item Powyższe~testy nie~uruchamiają kodu studenta.
\end{itemize}
